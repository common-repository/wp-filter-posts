...
if(!$_POST && (isset($_GET['wpf_notice'])&& $_GET['wpf_notice'] == 'hide')){
 
    if (! isset( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'],'wpf-shw')){
        wp_nonce_ays( 'wpf-shw');
        exit;
    } 
    update_option('xyz_wfp_dnt_shw_notice', "hide");
?>
...
<?php wp_nonce_field('fltr-sttng_'); ?>
	<table  class="widefat xyz_wpf_table" style="width:98%;padding-top: 10px;">
		...
		<tr valign="top">
			...
			<option value ="wpf" <?php if($xyz_credit_link=='wpf') echo 'selected'; ?> >Yes </option>
			<option value ="<?php echo $xyz_credit_link!='wpf'?$xyz_credit_link:0;?>" <?php if($xyz_credit_link!='wpf') echo 'selected'; ?> >No </option>
			</select> 
			</td>
		</tr>
		<tr>
